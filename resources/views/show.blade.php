            ...
            <main class="post blog-post col-lg-8">
                <div class="container">
                  <div class="post-single">
                    <div class="post-thumbnail"><img src="{{ asset($posts->featured) }}" alt="..." class="img-fluid"></div>
                    <div class="post-details">
                      <div class="post-meta d-flex justify-content-between">
                        <div class="category"><a href="{{ url('/blog/category/'.$posts->category->slug) }}">{{ $posts->category->title }}</a></div>
                      </div>

                      <h1>{{ $posts->title }}<a href="#"><i class="fa fa-bookmark-o"></i></a></h1>

                      <div class="post-footer d-flex align-items-center flex-column flex-sm-row"><a href="#" class="author d-flex align-items-center flex-wrap">
                          <div class="avatar"><img src="{{ asset($posts->user->avatar) }}" alt="..." class="img-fluid"></div>
                          <div class="title"><span>{{ $posts->user->name }}</span></div></a>
                        <div class="d-flex align-items-center flex-wrap">
                          <div class="date"><i class="icon-clock"></i> {{ $posts->published_at }}</div>
                          <div class="comments meta-last"><i class="icon-comment"></i>{{ $posts->comment->count() }}</div>
                        </div>
                      </div>
                      <div class="post-body">
                        {!! $posts->body !!}
                      ...
                      <div class="post-comments">
                        <header>
                          <h3 class="h6">Post Comments<span class="no-of-comments">{{  $posts->comment()->count() }}</span></h3>
                        </header>
                        @if ($posts->comment)
                      ...
                      <div class="add-comment">
                        ...
                        {!! Form::open(['route'=>['post.comment',$posts->slug],'method'=>'POST','class'=>'commenting-form']) !!}
                            {{ csrf_field() }}
                          <div class="row">
                            <div class="form-group col-md-6">
                          {!! Form::text('name',null,['class'=>$errors->has('name') ? 'form-control is-invalid' : 'form-control','placeholder'=>'enter your name']) !!}
                            @if ($errors->has('name'))
